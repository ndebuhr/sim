...
pub fn t_score(alpha: f64, df: usize) -> f64 {
    let alpha_index = ALPHAS
        .iter()
        .position(|alpha_option| utils::equivalent_f64(*alpha_option, alpha))
        .unwrap();
    if df > 100 {
        Z_SCORES[alpha_index]
    } else {
        T_SCORES[df - 1][alpha_index]
    }
}

const ALPHAS: [f64; 7] = [0.1, 0.05, 0.025, 0.01, 0.005, 0.001, 0.0005];

// https://www.calculators.org/math/z-critical-value.php
const Z_SCORES: [f64; 7] = [1.2816, 1.6449, 1.96, 2.3263, 2.5758, 3.0902, 3.2905];
